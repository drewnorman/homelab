...
radarr_lxc_vmid="${RADARR_LXC_VMID:-104}"

# Unprivileged LXC id mapping for container root:media on the Proxmox host.
remote_owner="${JELLYFIN_REMOTE_OWNER:-100000}"
remote_group="${JELLYFIN_REMOTE_GROUP:-101000}"
...
radarr_lxc_vmid_quoted="$(printf '%q' "$radarr_lxc_vmid")"
...
"${ssh_command[@]}" "$remote" \
  "if command -v pct >/dev/null && pct status $radarr_lxc_vmid_quoted | grep -q 'status: running'; then pct exec $radarr_lxc_vmid_quoted -- sh -c 'cd / && runuser -u radarr -- test -r \"\$1\" && runuser -u radarr -- test -x \"\$1\"' sh $remote_destination_dir; fi"
...
"${ssh_command[@]}" "$remote" \
  "df -h $remote_destination_dir"

printf 'Verifying Radarr can read the copied files...\n'
"${ssh_command[@]}" "$remote" \
  "if command -v pct >/dev/null && pct status $radarr_lxc_vmid_quoted | grep -q 'status: running'; then unreadable_path=\"\$(pct exec $radarr_lxc_vmid_quoted -- sh -c 'cd / && runuser -u radarr -- find \"\$1\" -type f ! -readable -print -quit' sh $remote_destination_dir)\"; if [ -n \"\$unreadable_path\" ]; then printf 'Radarr cannot read: %s\n' \"\$unreadable_path\" >&2; exit 1; fi; fi"

printf 'Checking Radarr library status...\n'
"${ssh_command[@]}" "$remote" \
  "if command -v pct >/dev/null && pct status $radarr_lxc_vmid_quoted | grep -q 'status: running'; then pct exec $radarr_lxc_vmid_quoted -- python3 -c 'import json, urllib.request, xml.etree.ElementTree as ET; api_key = ET.parse(\"/var/lib/radarr/config.xml\").findtext(\"ApiKey\"); request = urllib.request.Request(\"http://localhost:7878/api/v3/movie\", headers={\"X-Api-Key\": api_key}); movies = json.load(urllib.request.urlopen(request, timeout=30)); missing = [movie for movie in movies if not movie.get(\"hasFile\")]; print(\"Radarr movies: {} total, {} missing\".format(len(movies), len(missing))); [print(\"Missing: {} ({}) at {}\".format(movie.get(\"title\"), movie.get(\"year\"), movie.get(\"path\"))) for movie in missing[:10]]; print(\"...and {} more missing\".format(len(missing) - 10)) if len(missing) > 10 else None'; fi"
